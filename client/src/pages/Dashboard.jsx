...
const Tip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background:'#1e2535', border:'1px solid rgba(255,255,255,0.1)', borderRadius:8, padding:'8px 12px', fontSize:12 }}>
      <div style={{ color:'#8b95b0', marginBottom:4 }}>{label}</div>
      {payload.map((p,i) => <div key={i} style={{ color: p.color||'#ff6b35' }}>{p.name}: {typeof p.value === 'number' && p.value > 100 ? '₹'+p.value : p.value}</div>)}
    </div>
  )
}
          ...
          <div style={{ padding:'16px 20px' }}>
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={hourlyData}>
                ...
                <Tooltip content={<Tip />} />
                <Area type="monotone" dataKey="revenue" name="Revenue" stroke="#ff6b35" fill="url(#rg)" strokeWidth={2} dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div style={{ padding:'16px 20px' }}>
            <ResponsiveContainer width="100%" height={130}>
              <PieChart>
                ...
                <Tooltip contentStyle={{ background:'#1e2535', border:'1px solid rgba(255,255,255,0.1)', borderRadius:8, fontSize:12 }} />
              </PieChart>
            </ResponsiveContainer>
          ...
          <div style={{ padding:'16px 20px' }}>
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={weekly} barSize={16} barGap={4}>
                ...
                <Tooltip content={<Tip />} />
                <Bar yAxisId="rev" dataKey="revenue" name="Revenue" fill="#ff6b35" radius={[4,4,0,0]} />
                <Bar yAxisId="ord" dataKey="orders"  name="Orders"  fill="#3b82f6" radius={[4,4,0,0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
              ...
              <div style={{ marginTop:20, paddingTop:15, borderTop:'1px solid var(--border)' }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {invAlerts.slice(0,3).map((item,i) => (
                    <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', background:'rgba(231, 76, 60, 0.05)', padding:'6px 10px', borderRadius:6 }}>
          ...
          <div style={{ padding:'16px 20px' }}>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={catData} layout="vertical" barSize={10}>
                ...
                <Tooltip contentStyle={{ background:'#1e2535', border:'1px solid rgba(255,255,255,0.1)', borderRadius:8, fontSize:11 }} />
                <Bar dataKey="count" name="Items" fill="#ff6b35" radius={[0,4,4,0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
